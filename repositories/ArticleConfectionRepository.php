...
class ArticleConfectionRepository{
    ...
    public function updateArticleConfe(ArticleConfection $articleConfection):int{
        $sql="UPDATE `articleconfection` SET `qteStock` = '".$articleConfection->getQteStock()."' WHERE `articleconfection`.`id` = '".$articleConfection->getId()."';";
    ...
    public function selectArticleConfectionById(String $id):ArticleConfection|null{
        $sql="select * from articleconfection where id='$id'";
        try {
            $stmt = $this->database->getPdo()->query($sql);
            if($row = $stmt->fetch()){
    ...
    public function count():int{
        $sql="SELECT count(id) as count FROM `articleconfection`";
        try {
            $stmt = $this->database->getPdo()->query($sql);
            if($row = $stmt->fetch()){
            return $row["count"];
    ...
    public function selectLastInsertId():int{
        $sql="SELECT id FROM `articleconfection` ORDER by `id` desc LIMIT 0,1";
        try {
            $stmt = $this->database->getPdo()->query($sql);
            if($row = $stmt->fetch()){
            return $row["id"];

...
class ArticleVenteRepository{
    ...
    public function selectArticleVenteById(String $id):ArticleVente|null{
        $sql="select * from articlevente where id='$id'";
        try {
            $stmt = $this->database->getPdo()->query($sql);
            if($row = $stmt->fetch()){
    ...
    public function insertArticleVente(ArticleVente $articleVente):int{
        $sql= "INSERT INTO `articlevente` (`id`, `libelle`, `qteStock`, `prixVente`, `montVente`, `categorie`) VALUES ('".$articleVente->getId()."', '".$articleVente->getLibelle()."', '".$articleVente->getQteStock()."', '".$articleVente->getPrixVente()."', '".$articleVente->getMontVente()."', '".$articleVente->getCategorie()."');";
    ...
    public function count():int{
        $sql="SELECT count(id) as count FROM `articlevente`";
        try {
            $stmt = $this->database->getPdo()->query($sql);
            if($row = $stmt->fetch()){
            return $row["count"];
    ...
    public function selectLastInsertId():int{
        $sql="SELECT id FROM `articlevente` ORDER by `id` desc LIMIT 0,1";
        try {
            $stmt = $this->database->getPdo()->query($sql);
            if($row = $stmt->fetch()){
            return $row["id"];
